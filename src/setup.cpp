#pragma region 1
  #define ONLY_PRESSURE 0
  #define PRESSURE_AND_ELECTRICITY 1
  #define PRESSURE_AND_ULTRASONIC_FLOW 2
  #define ONLY_WATER_LEVEL 3
  #define USE_SLAVE 4
#pragma endregion 1

//---------------------SETUP PARAMETER------------------------------------------------------
//#define SOFTWARE_VERSION "B3.6.8m" //UNTUK BOARD YG BELUM DIMODIF
#define SOFTWARE_VERSION      "B3.7.8s" //UNTUK BOARD YG SUDAH DIMODIF
#define HARDWARE_VERSION      "BSA_IoT V3.5"

#define DEVICE_LOCATION       "Testing 8s" //SESUAIKAN DENGAN NAMA LOKASI ALAT
#define DEFAULT_TIMEZONE      7 //WIB=7, WITA=8, WIT=9
#define LCD_PRESSURE_OFFSET   0.0 //SESUAIKAN DENGAN NILAI OFFSET DI HARDWARE DOCS

#define _ADDON_               ONLY_PRESSURE 
/**PILIH SALAH SATU OPSI DI BAWAH INI UNTUK DICOPY KE #define _ADDON_:
 * ONLY_PRESSURE
 * PRESSURE_AND_ELECTRICITY
 * PRESSURE_AND_ULTRASONIC_FLOW
 * ONLY_WATER_LEVEL
 * USE_SLAVE
 */
//------------------END SETUP PARAMETER-----------------------------------------------------

#pragma region 
  #if _ADDON_ == PRESSURE_AND_ELECTRICITY
    #define _USE_PZEM_
    #define _DIRECT_PZEM_
  #elif _ADDON_ == PRESSURE_AND_ULTRASONIC_FLOW
    #define _USE_ULTRASONIC_FLOW_METER_
    #define _DIRECT_ULTRASONIC_FLOW_METER_
  #endif

  #define _OFFSET_P1_ LCD_PRESSURE_OFFSET 
  #define _OFFSET_P2_ LCD_PRESSURE_OFFSET 
  #define _OFFSET_P3_ LCD_PRESSURE_OFFSET 
#pragma endregion
