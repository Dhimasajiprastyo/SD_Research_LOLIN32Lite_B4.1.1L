...
#define DEFAULT_WATCHDOG_TIMER 240 // 5MENIT
#define DEFAULT_BATTERY_SAFE 11.1 //11 VOLT
#define DEFAULT_TIME_FOR_BROKER 90 //1.5 MENIT
...
//-----------opsi APN -------------------------------------------------------------------------
const char *filename_pend = "/pend";
const char *filename_sent = "/sent";
...
#ifndef _USE_ALL_PRESSURE_ //nanti ini disesuaikan lagi logikanya.. ini hanya untuk sementara
...
void lcd_clear_print(const char text[16], uint16_t t_delay){
...
float getAWLRvoltage(uint8_t AnalogPin) {
  ...
  uint8_t i;
  for(i=0;i<5;i++){
    analogRead = filterAnalog(AnalogPin);
    delay(50);
  }
...
void empty_sd(uint8_t max_delete){
  ...
  if(SDok()){
    if(sdop.countFile(SD, filename_pend, 1)>0){
      sdop.deleteManyFile(SD,filename_pend,50);
    }
    if(sdop.countFile(SD, filename_sent, 1)>0){
      sdop.deleteManyFile(SD,filename_sent,50);
    }
    if(sdop.countFile(SD, filename_pend, 1)==0){
      all_empty = true;
    }
...
bool SIM800SleepDisable() {
  ...
  while (i < 8) {
    ...
    if(i==7){
      int8_t btn = -1;
      lcd_clear_print("Save Time?",10);
      lcd.setCursor(0,1); lcd.print("No");
      lcd.setCursor(13,1); lcd.print("Yes");
      btn = wait_button(1);
      if(btn == 0){ //don't save
        lcd_clear_print("Time Cancled!",1000);
      }else if(btn == 1){ // save
...
String serializeData(Config configs, bool manual) { //PENGAMBILAN DATA SENSOR-----------
  ...
  digitalWrite(SW_DF_MODULE_PWR, 1);
  delay(1000);
  float h_awlr = getWaterHeight(1);
  delay(500);
  digitalWrite(SW_AWLR_PWR, 0);
  digitalWrite(SW_DF_MODULE_PWR, 0);
...
String report_text(String& add_text) {
  ...
  if(SPIFFS.exists(USING_DEF_RULES)||(!rules_exist)){
    buffer += "_R!";
  }
  if (!SDok()) {
    buffer += "_SD";
...
boolean mqttConnect(const char *broker, bool init_wdt, bool mqtt_connected, bool sd_halt) { //JANGAN LUPA KASI WATCHDOG TIMER DI SINI, KRN BISA SAJA MACET SAAT BACA SD----
  ...
  if (useSD) {
    jumlah_file_pending = sdop.countFile(SD, filename_pend, 50);
    if (!(SD.exists(filename_sent))) {
      sdop.createDir(SD, filename_sent);
    }
  ...
  for (i = 0; i < willsend; i++) {
    ...
    File root = SD.open(filename_pend);
...
uint16_t clr_sent_pend(bool sent, uint8_t total_delete, uint8_t auto_delete) {
  if (SDok()) {
    ...
    sent ? dir = filename_sent : dir = filename_pend;
...
void recountPend(uint16_t limited) {
  if (SDok()) {
    stopWDT();
    startWDT(600);
    if (!(SD.exists(filename_pend))) {
      sdop.createDir(SD, filename_pend);
    }
    lcd_clear_print("RECOUNT PEND...",10);
    if (limited > 0) {
      pending_aprox = sdop.countFile(SD, filename_pend, limited);
    } else {
      pending_aprox = sdop.countFile(SD, filename_pend, 0);
    }
...
void recountSent(uint8_t limited) {
  if (SDok()) {
    ...
    if (limited > 0) {
      recorded_aprox = sdop.countFile(SD, filename_sent, limited);
    } else {
      recorded_aprox = sdop.countFile(SD, filename_sent, 0);
    }
...
void log_data(bool manual) { //JANGAN LUPA KASI WATCHDOG TIMER KARENA MUNGKIN SAJA BISA HANG SAAT AKSES SD
  ...
  if (!disableSD_temporary) {
    ...
    uint8_t aprox_pending_data = sdop.countFile(SD, filename_pend, (max_pending_data + 5 ));
  ...
  for (ix = 0; ix < 5; ix++) {
    ...
    if (BatterySafeMode) {
      ...
      lcd_clear_print("SAFE MODE ON!!!",100);
    } else {
      lcd_clear_print("BATTERY OK!",100);
    }
  }
  ...
  // if (((cnt_for_auto_restart % 5) == 0) && (config.cnt_hard_reset > 0)) { //mekanisme auto hard reset ketika tidak ada sinyal selama periode tertentu
  //   if (!(wait_for_signal(30, 40))) { //hard reset berdasarkan kondisi sinyal
  //     do_hard_reset();
  //   }
  // }
...
void parsing_message(String& message) {
  ...
  // find = "sd_empty";
  // if (search_word(find, message) > -1) {
  //   if (check_similar_cmd(code_sd_empty, 0)) {
  //     return;
  //   }
  //   if (SDok()) {
  //     if(SPIFFS.exists("/sd_empty.txt")){
  //       mqtt.publish(STATUS_TOPIC, "{\"ans\":\"Empty SD Flag ALREADY SET!\"}");
  //     }else{
  //       sdop.writeFile(SPIFFS,"/sd_empty.txt","0");
  //       mqtt.publish(STATUS_TOPIC, "{\"ans\":\"Empty SD Flag IS SET!\"}");
  //     }dfsd;
  //   }
  //   return;
  // }
...
void setDefSettings() {
  ...
  config.max_send = 15;
  config.cnt_send_from_spiffs = 7; // setiap 7 kali pengambilan data, alat akan disable temporary SD dan mengirimkan data dari SPIFFS, sehingga bisa listening k broker
  config.SD_store_limit = 200; //(3hari)
...
void setup() {
  ...
  pinMode(SW_DF_MODULE_PWR, OUTPUT);
  digitalWrite(SW_AWLR_PWR, 0);
  digitalWrite(SW_DF_MODULE_PWR, 0);
#endif
...
*/
    ...
    setupConfig.timezone = DEFAULT_TIMEZONE;
    setupConfig.battery_safe = DEFAULT_BATTERY_SAFE;
    setupConfig.minimum_signal = 5;
    setupConfig.time_for_broker = DEFAULT_TIME_FOR_BROKER;
    setupConfig.watchdog_timer = DEFAULT_WATCHDOG_TIMER;
    setupConfig.useSD = true;
    setupConfig.max_send = 10;
    setupConfig.SD_attempt = 15;
    setupConfig.SD_store_limit = 20;
    setupConfig.cnt_send_from_spiffs = 7;
    setupConfig.sync_time_interval = 8;
    setupConfig.cnt_hard_reset = 2;
    ...
    while (1) { //------HANDLE MENU UTAMA--------------------------------------
      ...
      if (buttonEvt > -1 || initialFlag) {
        ...
        if (buffer == "#test$") {
          ...
          sdop.deleteFile(SPIFFS, USING_DEF_RULES);
          rules_exist = false;
  ...
  if (bAlarmOne) {
    ...
    if (!BatterySafeMode && (pending_aprox > 0)) {
      ...
      if (wait_for_signal(60, config.minimum_signal)) { //true = dapat sinyal, false = tdk dpt sinyal target
        ...
        if ((sdop.countFile(SD, filename_pend, 1)) > 0) { //cek ada file pending atau tidak
          filePending = true;
        }
    ...
    if (!wait_broker_initiated) {
      ...
      if (got_message) {
        ...
        if (cmd_message == cmd_sync_time) {
          ...
          uint16_t files = sdop.countFile(SD, filename_pend, sd_limit_access);
          ...
          uint16_t files = sdop.countFile(SD, filename_sent, sd_limit_access);
          files > 100 ? files = 100 : files;
          String answer; answer.reserve(34);
          answer = "{\"ans\":\"Sent:";
          answer += (String)files;
